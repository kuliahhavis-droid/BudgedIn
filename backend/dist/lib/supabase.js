...
export const getSupabaseAdmin = () => {
    ...
    if (!env.SUPABASE_URL) {
        throw new Error('SUPABASE_URL is not configured. Set it in the environment to use the Supabase admin client.');
    }
    if (!env.SUPABASE_SERVICE_ROLE_KEY) {
        throw new Error('SUPABASE_SERVICE_ROLE_KEY is not configured. Set it in the environment to use the Supabase admin client.');
    }
 ...
 */
export const getAvatarUploadUrl = async (userId, fileName) => {
    ...
    if (error || !data?.signedUrl) {
        throw new Error(`Failed to create signed upload URL for "${path}": ${error?.message ?? 'unknown error'}`);
    }

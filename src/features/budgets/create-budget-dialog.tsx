...
export function CreateBudgetDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent maxWidth="md">
        <DialogHeader>
          <DialogTitle>Tambah Budget Bulanan</DialogTitle>
          <DialogDescription>Atur batas pengeluaran bulan ini agar keuangan lebih terkontrol.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-6 pb-2">
            ...
            <div className="space-y-4">
              ...
              {form.formState.errors.categories?.message && (
                <p className="text-sm font-medium text-red-500">{form.formState.errors.categories.message as string}</p>
              )}
                ...
                {fields.map((field, index) => (
                  <div key={field.id} className="flex flex-col sm:flex-row items-start gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex items-start gap-2 w-full sm:w-auto">
                      ...
                      />
                      {fields.length > 1 && (
                        <Button type="button" variant="danger" className="mt-0 h-11 w-11 p-0 shrink-0" onClick={() => remove(index)}>
